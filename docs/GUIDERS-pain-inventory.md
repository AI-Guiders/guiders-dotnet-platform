...
| G-011 | Console `key=value` и slash `/path tail` — два resolve, один `commandId` | agent | CDP-style meta vs slash catalog | **Notations.Command.*** + **Argument.Kv** + **Argument.Delimited** + `invocation-parity-v1` vector ([ADR-0021](adr/GUIDERS-ADR-0021-notations-quarry-family.md)) | in-progress |
