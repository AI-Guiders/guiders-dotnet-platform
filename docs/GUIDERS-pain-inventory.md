...
| G-011 | Console `key=value` и slash `/path tail` — два resolve, один `commandId` | agent | CDP-style meta vs slash catalog | **Notations.Command.*** + **Argument.*** + conformance (`command-console`, `invocation-parity`, …) | done |
