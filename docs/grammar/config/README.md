...
2. **One grammar id per slot** — separate config documents per OS/profile or `when` (future).
3. Unknown id or wire mismatch → `config validate` error (`grammar-wire-mismatch`).
