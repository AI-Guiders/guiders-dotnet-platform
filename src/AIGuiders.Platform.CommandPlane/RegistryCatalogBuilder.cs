...
{
    public static IReadOnlyList<SlashCommandDescriptor> CollectDescriptors<TContext>(
        PlatformCommandRegistry<TContext> registry,
        Func<SlashCommandDescriptor, bool>? predicate = null)
    ...
    public static SlashCatalogIndex BuildIndex<TContext>(
        PlatformCommandRegistry<TContext> registry,
        Func<SlashCommandDescriptor, bool>? predicate = null)
        where TContext : ICommandContext =>
        SlashCatalogIndex.FromDescriptors(CollectDescriptors(registry, predicate));
