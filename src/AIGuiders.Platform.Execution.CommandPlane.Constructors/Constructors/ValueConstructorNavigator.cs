...
public sealed class ValueConstructorNavigator(
    ValueConstructorRegistry registry,
    IConstructorSegmentProvider segmentProvider)
{
    public IReadOnlyList<ArgCompletionItem> GetSuggestions(ArgConstructorDraft draft, string partial)
    {
    ...
    public bool TryAdvance(ArgConstructorDraft draft, string pickedValue)
    {
        if (!TryGetCurrentLeaf(draft, out var leaf, out var segmentIndex))
        {
            return false;
    ...
    public bool TryEmitWire(ArgConstructorDraft draft, out string wireValue, out string? error)
    {
        wireValue = "";
        error = null;
    ...
    public bool TryApplySegments(
        ...
        ValueConstructorRegistry registryInstance,
        LocaleInputProfile? profile)
    {
        ...
        {
            draft.DisplayBuffer = LocaleDisplayFormatter.FormatLeaf(leaf, draft.ActiveSegments, profile);
        }
    ...
    void CompleteCurrentLeaf(ArgConstructorDraft draft, LeafConstructorDefinition leaf)
    {
        ...
        draft.CompletedSlots.Add(new ArgConstructorDraft.CompletedSlot(slot.SlotId, segments));
    ...
    static string FormatDisplay(LeafConstructorDefinition leaf, IReadOnlyDictionary<string, string> segments)
    {
    ...
    static void AppendSeparator(ArgConstructorDraft draft, string wireSep, string displaySep)
    {
        draft.WireBuffer = string.IsNullOrEmpty(draft.WireBuffer)
            ? wireSep
    ...
    public bool TryGetCurrentLeaf(
        ArgConstructorDraft draft,
        out LeafConstructorDefinition leaf,
    ...
    public bool TryGetCurrentSlotLabel(ArgConstructorDraft draft, out string label)
    {
        label = "";
        var composite = registry.RequireComposite(draft.RootConstructorId);
        if (draft.SlotIndex >= composite.Slots.Count)
    ...
    static string FormatWire(LeafConstructorDefinition leaf, IReadOnlyDictionary<string, string> segments)
    {
